...
export default function Sidebar() {
  ...
  // show admin link only when user has publicMetadata.role === 'admin'
  const role = user?.publicMetadata?.role || user?.unsafeMetadata?.role;
  // also allow a small list of admin emails via VITE_ADMIN_EMAILS (comma-separated)
  const envList =
  ...
  const primaryEmail =
    ...
    user?.email;
  const isEmailAdmin =
    primaryEmail && adminEmails.has(String(primaryEmail).toLowerCase());

  if (role === "admin" || isEmailAdmin) {
    menuItems.splice(1, 0, { path: "/admin", icon: FiUsers, label: "Admin" });
  }
